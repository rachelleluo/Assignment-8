...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        Profile[] newArr = new Profile[count];
        ...
        for(int i = 0; i < count; i++){
            ...
            STR[] s1STR = null;
            STR[] s2STR = null;
            Profile newProf = new Profile(name, s1STR, s2STR, seq1, seq2);
            newArr[i] = newProf;
        }
        database = newArr.clone();
    }
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        for(int i = 0; i < count; i++){
            newArr[i] = StdIn.readLine();
        }

        STRsOfInterest = newArr.clone();
    }
	...
	/* WRITE YOUR CODE HERE */
        ...
        Profile unknownProf = new Profile("Unknown", s1STR, s2STR, seq1, seq2);

        return unknownProf; // update the return value
    }
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        ...
        if(STR.length() <= sequence.length()){
            for(int i = 0; i <= (sequence.length() - len); i++){
                String current = sequence.substring(i, i+len);
                if(current.equals(STR)){
                    numOfRepeats += 1;
                    for(int j = i + len; j <= (sequence.length() - len); j+=len){
                        String next = sequence.substring(j, j+len);
                        if(next.equals(STR)){
                            numOfRepeats += 1;
                        }else{
                            i = j;
    ...
    public void createProfileSTRs ( Profile profile, String[] allSTRs ) {
        ...
        /* WRITE YOUR CODE HERE */
        STR[] STR1 = new STR[allSTRs.length];
        STR[] STR2 = new STR[allSTRs.length];
    ...
    public ArrayList<Profile> findMatchingProfiles ( STR[] unknownProfileS1_STRs ) {
        ...
        /* WRITE YOUR CODE HERE */
        ArrayList<Profile> newArr = new ArrayList<>();
        for(int i = 0; i < database.length; i++){
            if(identicalSTRs(database[i].getS1_STRs(), unknownProfileS1_STRs)){
                newArr.add(database[i]);
            }
        }

	    return newArr; // update the return value
    }
        ...
        for ( int p1 = 0; p1 < possibleParent1.size(); p1++ ) {
           ...
           for ( int p2 = 0; p2 < possibleParent2.size(); p2++ ) {
               ...
               if ( !possibleParent1.get(p1).equals(possibleParent2.get(p2)) ) {
                   ...
                   if (punnetSquare(possibleParent2.get(p2).getS1_STRs(), S2_STRs, possibleParent1.get(p1).getS1_STRs(), S1_STRs)) {
